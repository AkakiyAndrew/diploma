...
int main(void)
{
    ...
    Image mainImage = LoadImage("avatar.png");
    ...
    int arrSize = grayImage.width * grayImage.height;
    
    Color* grayData = LoadImageColors(grayImage);
    int brightness;
    ...
    for (int i = 0; i < 256; i++)
    {
        levelsNormalised[i] = levels[i] / maxNum;
    }
    ...
    for (int i = 0; i < 256; i++)
    {
        ...
        int y = histZeropoint.y - (levelsNormalised[i] * histHeight)+1;
        dots[i].x = x;
        dots[i].y = y;
        DrawPixel(x, y, BLACK);
    }
    ...
    while (!WindowShouldClose())        // Detect window close button or ESC key
    {
        ...
        DrawRectangle(0, 0, mainTexture.width + grayTexture.width + 100, histZeropoint.y+50, RAYWHITE);
        ...
        // Histogram
        DrawLineEx(histZeropoint, histOrdinate, 2, DARKGRAY);
        DrawLineEx({ histOrdinate.x + 7, histOrdinate.y + 7 }, histOrdinate, 2, DARKGRAY);
        DrawLineEx({ histOrdinate.x - 7, histOrdinate.y + 7 }, histOrdinate, 2, DARKGRAY);
        DrawLineEx(histZeropoint, histAxis, 2, DARKGRAY);
        DrawLineEx({ histAxis.x - 7, histAxis.y - 7 }, histAxis, 2, DARKGRAY);
        DrawLineEx({ histAxis.x - 7, histAxis.y + 7 }, histAxis, 2, DARKGRAY);
        DrawLineStrip(dots, 256, BLACK);

        // Legend
        DrawText("0", histZeropoint.x-10, histZeropoint.y, 15, DARKGRAY);
        DrawText("255", histZeropoint.x + 245, histZeropoint.y + 5, 15, DARKGRAY);

        DrawText("0.25", histZeropoint.x - 35, histZeropoint.y-(histHeight/4), 15, DARKGRAY);
        DrawText("0.5", histZeropoint.x - 30, histZeropoint.y - (histHeight / 2), 15, DARKGRAY);
        DrawText("0.75", histZeropoint.x - 35, histZeropoint.y - (histHeight * 3 / 4), 15, DARKGRAY);
        DrawText("1.0", histZeropoint.x - 30, histZeropoint.y - histHeight , 15, DARKGRAY);
